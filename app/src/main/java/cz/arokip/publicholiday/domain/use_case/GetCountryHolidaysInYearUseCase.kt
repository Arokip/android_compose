...
class GetCountryHolidaysInYearUseCase @Inject constructor(
    private val repository: PublicHolidaysRepository,
) {
    operator fun invoke(year:String, countryCode: String): Flow<Resource<List<PublicHoliday>>> = flow {
        try {
            emit(Resource.Loading())
            val nextCountryHolidays = repository.getCountryHolidaysInYear(year = year, countryCode = countryCode)
